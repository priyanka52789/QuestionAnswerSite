        ...
        try {
            ...
            if (entry.level == LogLevel.Error) {
                if (!this._writeLogFailed) {
                    ...
                    let newLogItem: LogItem = new LogItem(this._applicationName, entry.data.FileName, entry.data.MethodName, new Date(), this._userId, this._userName, entry.message ? entry.message : "error occured", entry.data.Stack ? entry.data.Stack : "Cannot find stack trace", "Error Log");
